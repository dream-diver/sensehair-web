...
const FloatingWindowDate = ({
  ...
  const [minTime, setMinTime] = useState(
    setHours(setMinutes(new Date(startDate), 0), 10)
  );
  ...
  const fetchIncludeTimes = async (stylist, duration, date) => {
    try {
      const response = await fetch(
        ...
        }/api/servers/${stylist}/availableTimes?duration=${duration}&date=${format(
          startDate,
          "dd-MM-yyyy"
        )}`
      );
  ...
  return (
    <div className="floating-window">
      ...
      <div className="floating-window-footer">
        ...
        <a
          className="btn-next btn btn-dark"
          onClick={() => {
            validateDate(startDate) && nextStep();
          }}
        >
          {state.text.bookingNext}
